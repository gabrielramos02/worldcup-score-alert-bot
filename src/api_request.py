...
from database.manager import Live_Match, Team, add_team, get_subscription, get_team, get_teams
        ...
        async with session.get(url) as response:
            ...
            if html:
                ...
                for event in events:
                    ...
                    event_completed = event.get("status", {}).get("type", {}).get("completed", False)
                    ...
                    away_team_id = event.get("competitions", [{}])[0].get("competitors", [{}])[1].get("team", {}).get("id", "")
        ...
        async with session.get(url) as response:
            ...
            if html:
                ...
                for event in events:
                    ...
                    home_score = event.get("competitions", [{}])[0].get("competitors", [{}])[0].get("score", "")
                    away_score = event.get("competitions", [{}])[0].get("competitors", [{}])[1].get("score", "")
                    clock_time = event.get("competitions", [{}])[0].get("status", {}).get("displayClock", "")
                    LIVE_MATCHES.append(
                        Live_Match(
                            match_id=match_id,
                            home_team_id=home_team_id,
